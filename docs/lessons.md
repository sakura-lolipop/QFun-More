...
## §14 反汇编布尔方法：分支目标必须精确解析，否则语义整条读反

**现象**：RemoveSendPicLimit 移植后"选不上+弹上限提示"，症状与"解除限制"完全相反；
一整轮诊断（假选中/状态机分裂/hook 引擎怀疑）都建立在读反的语义上。

**根因**：V1()Z 方法体 `if-ge v0, v1, +004h; const/4 v0, 1; goto; const/4 v0, 0`——
`if-ge` 是**满足条件则跳过 const 1**，即 `(m+r) < q` 返回 true = **未满可继续选**。
移植时把跳转方向读反，注释成"isSelectFull 已选满"，于是恒 false = 恒"已满"——
每次点击必弹上限提示 + P1 拒收。QStory 的实现是 setResult(**TRUE**)，一字之差。

**正确做法**：
- 反汇编布尔方法的 if/goto，**逐条解析跳转目标**（androguard 指令序号 ≠ 字节码偏移，
  用分支语义而非序号加减），最好把整段写成伪代码再下结论；
- 移植型功能**必须核对原实现的返回方向**（QStory setResult 的值就是标准答案），
  不能从方法名/自己的注释臆断语义——"isSelectFull"这个名字是移植者起的，不是 QQ 的；
- 症状与功能目标**完全相反**时，第一反应该是"语义读反了"，而不是"QQ 状态机坏了"。
